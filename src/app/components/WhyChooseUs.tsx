import { motion } from "motion/react";
import { useState } from "react";
import implantImage from "../../assets/c08f04a06868f580b57ed869c8adf4babd9fa7dd.png";

export function WhyChooseUs({ lang = 'fr' }: { lang?: 'fr' | 'en' }) {
  const [hoveredIndex, setHoveredIndex] = useState<number | null>(null);

  const advantages = lang === 'fr' ? [
    {
      title: "Écoute Attentive",
      desc: "Nous prenons le temps de comprendre vos attentes uniques pour concevoir une solution adaptée.",
    },
    {
      title: "Haute Précision",
      desc: "Des technologies de pointe et un ajustement rigoureux pour un résultat parfaitement harmonieux.",
    },
    {
      title: "Confort Absolu",
      desc: "Votre confort en bouche est notre priorité, pour que vous oubliiez même votre prothèse.",
    },
    {
      title: "Solutions Personnalisées",
      desc: "Chaque sourire est différent, c'est pourquoi toutes nos prothèses sont entièrement personnalisées.",
    },
    {
      title: "Confiance & Sérénité",
      desc: "Une atmosphère détendue et un personnel qualifié pour des soins sans aucun stress.",
    }
  ] : [
    {
      title: "Careful Listens.",
      desc: "We take all the time to fully grasp your unique requests to design a solution that suits you.",
    },
    {
      title: "High Precisions",
      desc: "Cutting-edge technologies and a rigorous adjustments for a perfectly harmonious overall result.",
    },
    {
      title: "Total Comforts",
      desc: "Your mouth comfort is our priority, so that you will even forget about your prosthesis.",
    },
    {
      title: "Personalized Experiences",
      desc: "Every single smiles is completely different, which is why all our prostheses are so personalized.",
    },
    {
      title: "Trust & Serenity Now",
      desc: "A relaxed atmosphere and qualified staff for treatments without any kinds of stress.",
    }
  ];

  return (
    <section className="relative w-full min-h-screen bg-white text-[#1C1C1A] overflow-hidden z-20 flex flex-col justify-between pt-24 pb-16">
      <svg className="hidden w-0 h-0 absolute pointer-events-none">
        <filter id="liquid-glass" primitiveUnits="objectBoundingBox">
          <feImage
            result="map"
            width="100%"
            height="100%"
            x="0"
            y="0"
            href="data:image/webp;base64,"
          />
          <feGaussianBlur in="SourceGraphic" stdDeviation="0.04" result="blur" />
          <feDisplacementMap
            id="disp"
            in="blur"
            in2="map"
            scale="0.5"
            xChannelSelector="R"
            yChannelSelector="G"
          ></feDisplacementMap>
        </filter>
      </svg>
      
      {/* Animated Background Image - 100vh */}
      <div className="absolute inset-0 z-0 flex items-center justify-center pointer-events-none opacity-80">
        <motion.img
            animate={{ y: [-15, 15, -15] }}
            transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
            src={implantImage}
            alt="Implants dentaires 3D"
            className="w-full h-full object-cover object-center"
        />
      </div>

      <div className="relative z-10 w-full max-w-[1400px] mx-auto px-6 md:px-8 flex flex-col h-full flex-1 justify-between pb-8">
        
        {/* Header Area */}
        <div className="w-full flex justify-center mb-12 md:mb-16 mt-8">
          <h2 className="text-[40px] sm:text-[56px] md:text-[64px] lg:text-[80px] leading-[0.95] tracking-[-0.04em] font-medium text-[#1C1C1A] text-center">
            {lang === 'fr' ? "Plus de 15 ans d'" : "Over 15 years of "}<br className="md:hidden" /><span className="font-light italic text-[#58666F]">Expertise</span>.
          </h2>
        </div>

        {/* List Area */}
        <div className="w-full flex flex-col mt-auto max-w-[1200px] mx-auto gap-3 md:gap-4">
          {advantages.map((adv, idx) => {
            const isHovered = hoveredIndex === idx;
            
            return (
              <div 
                key={idx}
                className="group flex flex-col md:flex-row items-start md:items-center py-5 md:py-6 lg:py-8 px-6 md:px-10 border border-white/40 shadow-[0_8px_32px_rgba(0,0,0,0.04)] rounded-3xl cursor-pointer transition-all duration-300 relative z-10"
                onMouseEnter={() => setHoveredIndex(idx)}
                onMouseLeave={() => setHoveredIndex(null)}
              >
                {/* Liquid Glass Background */}
                <div 
                  className={`absolute inset-0 rounded-3xl overflow-hidden -z-10 transition-colors duration-300 backdrop-blur-md ${isHovered ? 'bg-[#F3F2EC]/80' : 'bg-[#F3F2EC]/65'}`}
                  style={{
                    backdropFilter: 'url(#liquid-glass)',
                    WebkitBackdropFilter: 'url(#liquid-glass)',
                  }}
                />

                {/* Number */}
                <div className="w-full md:w-[10%] mb-2 md:mb-0">
                  <span className={`text-[14px] md:text-[16px] font-medium tracking-widest transition-colors duration-300 ${isHovered ? 'text-[#58666F]' : 'text-[#1C1C1A]'}`}>
                    0{idx + 1}
                  </span>
                </div>

                {/* Title */}
                <div className="w-full md:w-[45%] mb-4 md:mb-0">
                  <h3 className={`text-[24px] md:text-[32px] lg:text-[40px] font-medium tracking-[-0.02em] transition-colors duration-300 ${isHovered ? 'text-[#58666F]' : 'text-[#1C1C1A]'}`}>
                    {adv.title}
                  </h3>
                </div>

                {/* Gap Column (for visual spacing to match reference structure) */}
                <div className="hidden md:block w-[15%]" />

                {/* Description */}
                <div className="w-full md:w-[30%]">
                  <p className={`text-[14px] md:text-[15px] leading-[1.6] transition-colors duration-300 ${isHovered ? 'text-[#1C1C1A]' : 'text-[#1C1C1A]/60'}`}>
                    {adv.desc}
                  </p>
                </div>
              </div>
            );
          })}
        </div>
      </div>
    </section>
  );
}